  ...
  def write
    ...
    input_csv.each do |row|
      ...
      if content_item.nil?
        ...
        output_csv << row.merge({
          "Pageviews Last 6 Months" => content_item[1],
          "Document Type" => content_item[0],
          "Primary Publishing Organisation" => content_item[2] || "UNKNOWN"
        })
      end
    end
...
private
  ...
  def fetch_all_the_things
    ...
    organisation_links = Content::Link
      ...
      .pluck(:source_content_id, :target_content_id, "content_items.title")
