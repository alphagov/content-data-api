...
RSpec.describe GA::Processor do
  ...
  context 'When the base_path matches the GA path' do
    before { allow_any_instance_of(GA::GoogleAnalyticsService).to receive(:find_in_batches).and_yield(ga_response) }
  ...
  context 'when page_path starts "/https://www.gov.uk"' do
    it 'removes the "/https://www.gov.uk" from the GA::Event page_path' do
      allow_any_instance_of(GA::GoogleAnalyticsService).to receive(:find_in_batches).and_yield(ga_response_with_govuk_prefix)
