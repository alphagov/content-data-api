require 'digest/sha1'
RSpec.describe Content::Parsers::Metadata::ContentHash do
  ...
  it "returns the hash of the content as 'content_hash'" do
    allow(Content::Parsers::ContentParser).to receive(:extract_content).and_return(content)

    expect(subject.parse(raw_json)).to eq(content_hash: Digest::SHA1.hexdigest(content))
    expect(Content::Parsers::ContentParser).to have_received(:extract_content).with(raw_json)
  end
  ...
  it 'returns { content_hash: nil } if JSON is provided by it has no content' do
    allow(Content::Parsers::ContentParser).to receive(:extract_content).and_return(nil)
