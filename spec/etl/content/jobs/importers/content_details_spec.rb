RSpec.describe Content::Jobs::Importers::ContentDetails do
  ...
  context 'Import contents' do
    subject { Content::Jobs::Importers::ContentDetails.new(latest_dimension_item.id) }
    let(:locale) { 'en' }
    ...
    before do
      ...
      allow(Content::Jobs::ImportQualityMetricsJob).to receive(:perform_async)
      allow(Content::Parsers::MetadataParser).to receive(:parse).and_return(
        raw_json: raw_json,
        number_of_pdfs: 99,
    ...
    context "when the locale is 'en'" do
      ...
      context 'and the content has changed' do
        ...
        it 'triggers a quality metrics job' do
          subject.run
          expect(Content::Jobs::ImportQualityMetricsJob).to have_received(:perform_async).with(latest_dimension_item.id)
          expect(Content::Jobs::ImportQualityMetricsJob).not_to have_received(:perform_async).with(latest_dimension_item_fr.id)
        end
      end
      ...
      context 'and the content has not changed' do
        ...
        it "doesn't run a quality metrics job" do
          subject.run
          expect(Content::Jobs::ImportQualityMetricsJob).not_to have_received(:perform_async)
        end
      end
    end
    ...
    context "when the locale is not 'en' and the content has changed" do
      ...
      it "doesn't run a quality metrics job" do
        subject.run
        expect(Content::Jobs::ImportQualityMetricsJob).not_to have_received(:perform_async).with(latest_dimension_item.id)
        expect(Content::Jobs::ImportQualityMetricsJob).not_to have_received(:perform_async).with(latest_dimension_item_fr.id)
      end
    end
  end
  ...
  context 'when GdsApi::HTTPGone is raised' do
    subject { Content::Jobs::Importers::ContentDetails.new(existing_content_item.id) }
    let(:locale) { 'en' }
