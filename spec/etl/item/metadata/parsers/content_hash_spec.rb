require 'digest/sha1'
RSpec.describe Item::Metadata::Parsers::ContentHash do
  ...
  it "returns the hash of the content as 'content_hash'" do
    allow(Item::Content::Parser).to receive(:extract_content).and_return(content)

    expect(subject.parse(raw_json)).to eq(content_hash: Digest::SHA1.hexdigest(content))
    expect(Item::Content::Parser).to have_received(:extract_content).with(raw_json)
  end
  ...
  it 'returns { content_hash: nil } if JSON is provided by it has no content' do
    allow(Item::Content::Parser).to receive(:extract_content).and_return(nil)
