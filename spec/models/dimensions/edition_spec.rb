RSpec.describe Dimensions::Edition, type: :model do
  let(:now) { Time.zone.new(2018, 2, 21, 12, 31, 2) }
  ...
  describe '#metadata' do
    ...
    it 'returns the correct attributes' do
      expect(edition.reload.metadata).to eq(
        ...
        locale: 'en',
        title: 'The Title',
        first_published_at: Time.zone.new(2018, 1, 1).strftime("%Y-%m-%d"),
        public_updated_at: Time.zone.new(2018, 5, 20).strftime("%Y-%m-%d"),
        publishing_app: 'publisher',
        document_type: 'guide',
        primary_organisation_title: 'The ministry',
        withdrawn: false,
