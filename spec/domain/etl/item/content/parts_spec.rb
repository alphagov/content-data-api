RSpec.describe Etl::Item::Content::Parser do
  ...
  describe "Parts" do
    ...
    it "returns nil if the part slug is not present in the parts hash" do
      ...
      expect(subject.extract_content(json.deep_stringify_keys, subpage_path: 'foo')).to eq(nil)
    end
    ...
    it "returns body text when extracting content for a single part" do
      ...
      expect(subject.extract_content(json.deep_stringify_keys, subpage_path: 'schools')).to eq('Local council')
    end
  end
end
