...
RSpec.describe Etl::GA::InternalSearchProcessor do
  ...
  context "When the GA path contains the gov.uk prefix" do
    before { allow(Etl::GA::InternalSearchService).to receive(:find_in_batches).and_yield(ga_response_with_govuk_prefix) }
    ...
    context "when an event already exists with the same page_path" do
      ...
      it "does not update metrics for other days" do
        edition1 = create :edition, date: "2018-02-20", base_path: "/Path1"
        fact1 = create :metric, edition: edition1, date: "2018-02-20", searches: 1
      ...
      it "does not update metrics for other items" do
        edition = create :edition, base_path: "/non-matching-path", date: "2018-02-20"
        fact = create :metric, edition:, date: "2018-02-20", searches: 1

        described_class.process(date:)

        expect(fact.reload).to have_attributes(searches: 1)
      end
